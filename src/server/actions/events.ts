...
import { eventFormSchema, EventFormType } from "@/schema/events";
import { auth } from "@clerk/nextjs/server";
import { and, eq } from "drizzle-orm";
import { redirect } from "next/navigation";
import "use-server";

export async function createEvent(unsafeData: EventFormType): Promise<
  | {
      error: boolean;
    }
  | undefined
...
export async function updateEvent(id:string,unsafeData: EventFormType): Promise<
  | {
      error: boolean;
    }
  | undefined
